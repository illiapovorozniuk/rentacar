...
@section('style')
    <link rel="stylesheet" href="/css/front/order.css">
@endsection
{{--@section('title')--}}
{{--    {{ $title }}--}}
{{--@endsection--}}
        ...
        <div class="container_with_separators">
            ...
            <div class="order_container">
                ...
                <div class="end_section">
                    ...
                    @endif
                    <div>{{\Carbon\Carbon::parse($order->date_from)->locale(App::getLocale())->isoFormat('D MMMM Y')}}{{$order->date_from !==$order->date_to?' – '. \Carbon\Carbon::parse($order->date_to)->locale(App::getLocale())->isoFormat('D MMMM Y'):''}}</div>
                    <div class="address">🏁:
                        @if($order->address_from)
                            {{$order->address_from}}
                        @else
                            <a href="https://maps.google.com/maps?q={{$car->latitude}},{{$car->longitude}}"
                               target="_blank">🗺️</a>
                        @endif
                    </div>
                    <div class="address">📍:
                    ...
                    @elseif($order->payment_status !== \App\Enums\OrderType::PAYMENT_PAID->value)
                        <div class="not_payed_container">
                            ...
                            <div id="payment-timer" data-deadline="{{ \Carbon\Carbon::parse($order->created_at)->addMinutes(10)->timestamp }}"></div>
                            {!! $form !!}
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (!timerDiv) return;
        const deadline = parseInt(timerDiv.dataset.deadline, 10) * 1000;
        function updateTimer() {
            const now = Date.now();
            let diff = Math.max(0, Math.floor((deadline - now) / 1000));
            const min = String(Math.floor(diff / 60)).padStart(2, '0');
            const sec = String(diff % 60).padStart(2, '0');
            timerDiv.textContent = diff > 0
                ? `{{trans('front.order.time_to_pay')}}: ${min}:${sec}`
                : '{{trans('front.order.payment_time_expired')}}';
